...
document
	...
	.addEventListener('click', searchAndReceiveRecipes)
...
function searchAndReceiveRecipes() {
	...
	if (document.getElementById('search-terms-input').value) {
		searchDataToSend.searchTerms = document
			.getElementById('search-terms-input')
			.value.split(' ')
	}
	const sortRelation = {
		Upvotes: 'upvotes',
	...
	searchDataToSend.numberToReceive = Number(
		document.getElementById('how-many-to-show-slider').value
	)
	...
	APIFetchJSON.then((receivedData) => {
		...
		if (receivedData == 'search returned nothing') {
			document.getElementById('new-recipes-div').querySelector('h3').innerHTML =
				'No results found'
			setTimeout(() => {
				document
					.getElementById('new-recipes-div')
					.querySelector('h3').innerHTML = 'Search results:'
			}, 2000)
